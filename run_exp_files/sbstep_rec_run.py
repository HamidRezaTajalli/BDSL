...
# Parameter lists based on step_by_step_rec.py arguments
...
# Additional parameter lists for attack parameters
attack_list = ["badnet", "blend", "sig"]
attack_list = ["badnet"]
trigger_size_list = [0.08]
attack_mode_list = ["all-to-one"]

# Backbone freeze parameter
backbone_freeze_rounds_list = [0]
                                    ...
                                    for attack in attack_list:
                                        for trigger_size in trigger_size_list:
                                            for attack_mode in attack_mode_list:
                                                for backbone_freeze_rounds in backbone_freeze_rounds_list:
                                                    job_script_file = f"step_by_step_rec_exp_{exp_num}_{dataset}_{model}_{num_clients}_{num_rounds}_{epochs_per_client}_{cut_layer}_{poisoning_rate}_{target_label}_{attack}_{trigger_size}_{attack_mode}_{backbone_freeze_rounds}.sh"
