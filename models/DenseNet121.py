...
class DenseNet121Decoder(nn.Module):
    ...
    Note: Backbone outputs flattened features [B, 1024] after avgpool + flatten.
    ...
    def __init__(self, cut_layer, input_size=(224, 224)):
        ...
        # DenseNet121 backbone always outputs [B, 1024] after avgpool + flatten
        ...
        self.initial_channels = 1024  # Use 1024 channels for spatial representation
        
        # Project flattened features to spatial representation
        # [B, 1024] -> [B, 1024*7*7]
        self.fc_projection = nn.Sequential(
            nn.Linear(1024, self.initial_channels * self.initial_size * self.initial_size),
            nn.ReLU(inplace=True)
        )
        
        # Build decoder architecture - same for all cut layers since backbone output is always [B, 1024]
        # Input: [B, 1024, 7, 7] after projection & reshape -> Upsample to [B, 3, 224, 224]
        self.decoder = nn.Sequential(
            # Start from [B, 1024, 7, 7]
            # Upsample to 14x14
    ...
    def forward(self, backbone_output):
        ...
        Args:
            backbone_output: Flattened features from backbone [B, 1024]
        ...
        # Project from [B, 1024] to [B, 1024*7*7]
        projected = self.fc_projection(backbone_output)
        
        # Reshape to [B, 1024, 7, 7]
